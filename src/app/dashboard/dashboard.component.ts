...
export class DashboardComponent implements OnInit {
  ...
  public pieChartData: ChartData<'pie', number[], string | string[]> = {
    ...
    datasets: [{
      data: [],
      backgroundColor: ['#6c63ff','#764ba2','#667eea','#9c88ff','#a29bfe','#74b9ff','#0984e3','#00cec9']
    }]
  };
  ...
  public barChartData: ChartData<'bar'> = {
    ...
    datasets: [
      { data: [], label: 'Inscriptions', backgroundColor: '#6c63ff' }
    ]
  };
  ...
  updateCharts(data: any): void {
    ...
    if (data.events_by_status) {
      this.doughnutChartData = {
        ...
        datasets: [{
          data: [
            data.events_by_status['actif'] || 0,
            data.events_by_status['archivé'] || data.events_by_status['archive'] || 0
          ],
          backgroundColor: ['#6c63ff', '#b0bec5'],
          borderWidth: 3,
          borderColor: ['#ffffff', '#ffffff']
    ...
    if (data.registrations_by_month) {
      ...
      this.barChartData = {
        labels: ['Jan','Fév','Mar','Avr','Mai','Jun','Jul','Aoû','Sep','Oct','Nov','Déc'],
        datasets: [{ data: months, label: 'Inscriptions', backgroundColor: '#6c63ff' }]
      };
    }
  }
}